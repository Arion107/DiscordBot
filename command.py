...
async def check_command_limit(interaction: discord.Interaction):
    ...
    if user.id in users and users[user.id] >= 25:
        await interaction.response.send_message(f"{user.mention}, Your free dreams have come to an end. Please consider subscribing to one of our plans.To see more details, use the /subscribe command or visit our website at https://mee6.gg/m/thedream", ephemeral=True)
        return False
    return True
...
async def generate_dream(interaction: discord.Interaction, task: str):
    try:
        ...
        await interaction.response.send_message(f" **{task}** - {interaction.author.mention}", file=File(image, "dream.png"))
    except Exception as e:
        await interaction.response.send_message(f"{interaction.author.mention}, The bot is having technical problems, in a few moments it will be operational again.", ephemeral=True)
        print(e)
...
async def start_bot():
    while True:
        try:
            ...
            @bot.interaction()
            async def dream(ctx: discord.Interaction, task: str):
                queue.append((ctx,task))
                await ctx.response.defer()
                await ctx.followup.send("Dreaming...", ephemeral=True)
                
            @bot.interaction()
            async def subscribe(ctx: discord.Interaction):
                await ctx.response.send_message("Open the page below to pick your plan and subscribe! https://mee6.gg/m/thedream", ephemeral=True)
